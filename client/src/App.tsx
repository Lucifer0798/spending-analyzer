...
import { fetchAccounts, resetAllData } from "./api";
...
function App() {
  ...
  // The account filter only applies to views that show transaction data.
  const showAccountFilter = tab !== "manage" && tab !== "upload" && accounts.length > 0;
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950">
      ...
      <main key={refreshKey}>
        ...
        {tab === "dashboard" && <Dashboard accountId={accountId} />}
        {tab === "transactions" && <TransactionsTable accountId={accountId} />}
        {tab === "recurring" && <RecurringPage accountId={accountId} />}
