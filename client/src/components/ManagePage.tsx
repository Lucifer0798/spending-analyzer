...
export function ManagePage({ onAccountsChanged }: Props) {
  ...
  const reload = async () => {
    const [accountsRes, categoriesRes, merchantsRes, budgetsRes, recurringOverridesRes, tagsRes] =
      await Promise.all([
        fetchAccounts(true),
        fetchCategories(),
        fetchMerchants(),
  ...
  const handleImportFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (
      !confirm(
        "This deletes everything currently in this instance — accounts, transactions, categories, budgets, merchant memory, recurring flags, savings goals, and tags — and replaces it with the backup file. This cannot be undone. Continue?"
      )
    ) {
      return;
    }
    ...
    try {
      ...
      setNotice(
        `Restored ${summary.accounts} accounts, ${summary.transactions} transactions, ` +
          ...
          `${summary.goals} savings goals, ${summary.tags} tags.`
      );
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      <section className="mt-10">
        ...
        <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
          Free-form labels set from Transactions, for filtering that cuts across categories — "business
          trip" or "reimbursable" alongside whatever categories those transactions already have.
        </p>

        {tags.length === 0 ? (
          <p className="mt-4 rounded-lg bg-slate-50 p-6 text-center text-sm text-slate-500 dark:bg-slate-900">
            Nothing tagged yet. Add a tag to a transaction from the Transactions page.
          </p>
        ) : (
          <div className="mt-4 overflow-hidden rounded-lg border border-slate-200 dark:border-slate-800">
            <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-800">
              <tbody className="divide-y divide-slate-100 bg-white dark:divide-slate-800 dark:bg-slate-950">
                {tags.map((t) => (
                  <tr key={t.name}>
                    <td className="px-4 py-2">
                      <div className="text-sm text-slate-800 dark:text-slate-200">{t.name}</div>
                      <div className="text-xs text-slate-500">
                    ...
                    <td className="px-4 py-2 text-right">
                      <button
                        onClick={() => {
                          if (
                            confirm(
                              `Delete the tag "${t.name}"? It will be removed from ${t.count} transaction${t.count === 1 ? "" : "s"}.`
                            )
                          ) {
                            run(() => deleteTag(t.name), `Deleted "${t.name}".`);
                          }
                        }}
      ...
      <section className="mt-10 mb-10">
        ...
        <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
          ...
          merchant memory, recurring flags, savings goals, and tags — for keeping a backup or
          moving to a new instance. AI forecasts aren't included; regenerating one is one click.
        </p>
